...
\subsection{Marking a new version}
\begin{enumerate}
    \item Edit \texttt{src/jolt/\_\_init\_\_.py}, and set \texttt{\_\_version\_\_} to the new version number, eg "1.0.3".
    Commit this change.
    \item Tag the commit with a corresponding name, but starting with a "v", eg "v1.0.3".
    In the tag text, put the changelog.
    \item You can now build the release files.
    \item After the build, you can update again the \texttt{\_\_version\_\_} to a development version,
    eg, "1.0.3-dev".
\end{enumerate}
